...
* **You**: "You will then be prompted to enter a remote URL that locates the YAML file declaring the details of the new component. Use this URL: `https://github.com/relessawy/backstage_entity_examples/blob/main/catalog-info.yaml`. Click **Analyze**. Finally, click **Import**. RHDH will then fetch the YAML file, parse it, and populate the **Catalog**."
...
"While Organizational Entities can be:"
...
** The component named 'globex-backend' provides an API named 'globex-api,' and has a dependency on a database called 'globex-database'. 
...
Homer Simpson: "So, it’s like a family tree, but for computer stuff? I guess every family has its nuts!"
...
1. **Log into the OpenShift cluster as the admin user using the `oc` CLI.**
2. **Switch to the `rhdh` namespace where you installed RHDH.**
3. **Create a file named `env.sh` with the following environment variables, replacing the values with the GitHub App details you created in the previous steps. Replace the `GITHUB_APP_PRIVATE_KEY_FILE` with your private key file that you downloaded from GitHub.**
...
export GITHUB_ORGANIZATION='rhdh-team0'
export GITHUB_ORG_URL=$GITHUB_HOST_DOMAIN/$GITHUB_ORGANIZATION
export GITHUB_APP_ID='960935'
export GITHUB_APP_PRIVATE_KEY_FILE="$(< <name of private key file>.pem)"
export GITHUB_APP_WEBHOOK_URL=<Your RHDH Route URL>
export GITHUB_APP_WEBHOOK_SECRET=7opS3cr3tK3y
```

4. **Source the `env.sh` file to export the environment variables:**

```bash
source env.sh
```

5. **Execute the following command to add keys related to GitHub authentication:**
...
* **You**: "You need to create a new ConfigMap named `dynamic-plugins-rhdh-local` to enable the **GitHub** related plugins. Usr the following YAML snippet:"
